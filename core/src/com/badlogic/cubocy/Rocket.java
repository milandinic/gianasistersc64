...
public class Rocket {
	...
	public void update(float deltaTime) {
		if (state == FLYING) {
			// if(pos.dst(map.bob.pos) < pos.dst(map.cube.pos))
			// vel.set(map.bob.pos);
			// else vel.set(map.cube.pos);
			vel.set(map.giana.pos);
	...
	Rectangle[] r = { new Rectangle(), new Rectangle(), new Rectangle(),
			new Rectangle() };
	...
	private boolean checkHit() {
		...
		if (bounds.overlaps(map.giana.bounds)) {
