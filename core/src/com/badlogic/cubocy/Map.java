	...
	private void loadBinary() {
		...
		for (int y = 0; y < 35; y++) {
			for (int x = 0; x < 150; x++) {
				...
				if (match(pix, START)) {
					Dispenser dispenser = new Dispenser(x, pixmap.getHeight()
							- 1 - y);
					dispensers.add(dispenser);
					activeDispenser = dispenser;
					bob = new Giana(this, activeDispenser.bounds.x,
							activeDispenser.bounds.y);
					bob.state = Giana.SPAWN;
					...
					Rocket rocket = new Rocket(this, x, pixmap.getHeight() - 1
							- y);
					rockets.add(rocket);
				} else if (match(pix, MOVING_SPIKES)) {
					movingSpikes.add(new MovingSpikes(this, x, pixmap
							.getHeight() - 1 - y));
